...
// its row and column position. All cursor markers are removed from lines after extraction.
...
func extractCursorPosition(lines []string, height int) (int, int) {
	...
	for row := len(lines) - 1; row >= viewportTop; row-- {
		...
		for {
			...
			if cursorRow == -1 {
				cursorRow = row
				cursorCol = VisibleWidth(line[:index])
			}
			line = line[:index] + line[index+len(CursorMarker):]
		}
		lines[row] = line
	}
